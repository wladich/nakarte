...
import {MountainPassesMarkers} from './mountainPassesMarkers';

L.Layer.MountainPasses = L.Layer.extend({
        options: {
            filePasses: 'passes.json',
            fileCoverage: 'coverage.json',
            fileLabels1: 'regions_labels1.json',
            fileLabels2: 'regions_labels2.json',
            scaleDependent: true,
            labels2Zoom: 6,
            markersZoom: 10,
            labels1Zoom: 2
        ...
        MarkersClass: MountainPassesMarkers,

        initialize: function(baseUrl, options) {
            L.setOptions(this, options);
            this.markers = new this.MarkersClass(baseUrl + this.options.filePasses, options.markersOptions);
            this.coverage = new L.Layer.GeoJSONAjax(baseUrl + this.options.fileCoverage, {
                className: 'mountain-coverage-polygon',
        ...
        _makeMarker: function(geojsonPoint, latlng) {
            const icon = L.divIcon({
                    className: 'mountain-region-label',
